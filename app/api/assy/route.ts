...
export async function POST(request: Request) {
  try {
    ...
    const { assy_code, assy_number, sequence, carline, destinasi, komoditi, prod_qty, description, is_active } = body;
    ...
    const result = await pool.query(
      `INSERT INTO master_assy (assy_code, assy_number, sequence, carline, destinasi, komoditi, prod_qty, description, is_active)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       ON CONFLICT (assy_code) DO NOTHING
       RETURNING *`,
      [assy_code, assy_number, sequence ?? null, carline ?? null, destinasi ?? null,
       komoditi ?? null, prod_qty ?? null, description ?? null, is_active ?? true]
    );
