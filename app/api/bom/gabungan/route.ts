...
// GET /api/bom/gabungan?dari=2025-10&sampai=2026-09&mode=list&page=1&limit=50&search=
  ...
  try {
    ...
    if (mode === 'list') {
      ...
      const dataParams = search
        ? [dari, sampai, `%${search}%`, limit, offset]
      ...
      const result = await pool.query(`
        SELECT
          b.part_no,
          b.assy_code,
          SUM(b.qty_per_unit) AS total_qty,
          COUNT(DISTINCT b.periode) AS periode_count,
          mp.part_name,
          mp.unit,
          mp.supplier_name
        FROM bom_detail b
        ...
        ${search ? `AND (b.part_no ILIKE $3 OR b.assy_code ILIKE $3)` : ''}
        GROUP BY b.part_no, b.assy_code, mp.part_name, mp.unit, mp.supplier_name
      ...
      // Ambil assy codes yang ada di range ini
      ...
      const countResult = await pool.query(`
        SELECT COUNT(DISTINCT part_no) FROM bom_detail
        WHERE periode >= $1 AND periode <= $2 ${searchWhere}
      `, countParams);
      ...
      // SUM qty per part per assy untuk part yang ada di halaman ini
      const qtyResult = await pool.query(`
        SELECT part_no, assy_code, SUM(qty_per_unit) AS total_qty
        FROM bom_detail
      ...
      for (const row of qtyResult.rows) {
        ...
        qtyMap[row.part_no][row.assy_code] = Number(row.total_qty);
      }
