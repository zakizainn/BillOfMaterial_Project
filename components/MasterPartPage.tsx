      ...
      {preview && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ maxHeight: 260, overflowY: 'auto', borderRadius: 10, border: '1px solid #e2e8f0' }}>
            <Table
              headers={[{label:'Part No'},{label:'Part No AS400'},{label:'Part Name'},{label:'Unit'},{label:'Supplier'},{label:'Status'}]}
              rows={preview.map(r => [r.part_no, r.part_no_as400, r.part_name, r.unit, r.supplier_name || '—', <Badge active={!!r.is_active} />])}
            />
          </div>
        </div>
      )}
  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
        <Field label="Part No" required error={errors.part_no}>
          <Input value={form.part_no} onChange={e => set('part_no', e.target.value)} placeholder="P-001" disabled={editing} />
        </Field>
        <Field label="Part No AS400" required error={errors.part_no_as400}>
          <Input value={form.part_no_as400} onChange={e => set('part_no_as400', e.target.value)} placeholder="AS4-1001" />
        </Field>
      </div>
      <Field label="Part Name" required error={errors.part_name}>
        <Input value={form.part_name} onChange={e => set('part_name', e.target.value)} placeholder="e.g. Bolt M8 x 20" />
      </Field>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
        <Field label="Unit" required error={errors.unit}>
          <Select value={form.unit} onChange={e => set('unit', e.target.value)} options={UNIT_OPTIONS} />
        </Field>
        <Field label="Supplier Code">
          <Input value={form.supplier_code ?? ''} onChange={e => set('supplier_code', e.target.value)} placeholder="PT A" />
        </Field>
      </div>
      <Field label="Supplier Name">
        <Input value={form.supplier_name ?? ''} onChange={e => set('supplier_name', e.target.value)} placeholder="PT Maju Jaya" />
      </Field>
      <Field label="Status">
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function MasterPartPage({ showToast }: { showToast: (msg: string, type: 'success' | 'error') => void }) {
  ...
  return (
    <div style={{ fontFamily: font }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              placeholder="Cari part no / nama / supplier..."
          ...
          <select value={filterUnit} onChange={e => { setFilterUnit(e.target.value); setPage(1); }} style={{
      ...
      {loading ? <LoadingSpinner /> : (
        <>
          <Table
            headers={[{label:'Part No'},{label:'Part No AS400'},{label:'Part Name'},{label:'Unit'},{label:'Supplier Code'},{label:'Supplier Name'},{label:'Status'},{label:'Aksi'}]}
            rows={tableRows}
          />
      ...
      {modal === 'add' && <Modal title="Tambah Part Baru" onClose={() => setModal(null)}><PartForm onSave={handleAdd} onClose={() => setModal(null)} existingPartNos={data.map(r => r.part_no)} /></Modal>}
      {modal && typeof modal === 'object' && 'editing' in modal && <Modal title={`Edit — ${modal.editing.part_no}`} onClose={() => setModal(null)}><PartForm initial={modal.editing} onSave={handleEdit} onClose={() => setModal(null)} existingPartNos={data.map(r => r.part_no)} /></Modal>}
      {confirm !== null && <ConfirmDialog msg={`Yakin ingin menghapus ${data.find(r => r.id === confirm)?.part_no} — ${data.find(r => r.id === confirm)?.part_name}? Data tidak dapat dikembalikan.`} onConfirm={() => handleDelete(confirm)} onCancel={() => setConfirm(null)} />}
